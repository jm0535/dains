...
if("red_list_category" %in% colnames(plants) & "group" %in% colnames(plants)) {
  ...
  chi_sq_residuals_df <- data.frame(
    `Red List Category` = rownames(chi_sq_residuals),
    `Plant Group` = colnames(chi_sq_residuals),
    Residual = round(chi_sq_residuals, 2)
  )
  chi_sq_residuals_long <- pivot_longer(chi_sq_residuals_df, 
                                        cols = -`Red List Category`, 
                                        names_to = "Plant Group", 
                                        values_to = "Residual")
  
  # Display the residuals table
  knitr::kable(chi_sq_residuals_long, 
