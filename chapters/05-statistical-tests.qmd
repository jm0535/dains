...
if("red_list_category" %in% colnames(plants) & "group" %in% colnames(plants)) {
  ...
  residuals_df <- as.data.frame(chi_sq_residuals)
  
  # Add category names as a column
  residuals_df$Category <- rownames(residuals_df)
  
  # Reshape to long format for better display
  residuals_long <- tidyr::pivot_longer(
    residuals_df,
    cols = -Category,
    names_to = "Group",
    values_to = "Residual"
